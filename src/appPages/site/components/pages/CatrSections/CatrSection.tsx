...
const CartSection = () => {
 ...
 const handleUpdateQuantity = async (itemId: number, quantity: number) => {
   ...
   try {
     ...
     const sameItems = basketData.filter(item => 
       item.clothes.clothes_name === currentItem?.clothes.clothes_name && 
       item.color === currentItem?.color &&
       item.id !== itemId
     );

     if (sameItems.length > 0) {
       const totalQuantity = sameItems.reduce((total, item) => total + item.quantity, 0) + quantity;
       ...
       await updateMutation({
         id: itemId,
         updateBasket: { quantity: totalQuantity }
       });
     } else {
       await updateMutation({
         id: itemId,
